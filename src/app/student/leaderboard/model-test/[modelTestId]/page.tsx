...
import { Trophy, Clock, Loader2, ArrowLeft, Medal, User, Crown, Star, Calendar, FileBadge } from 'lucide-react';
...
export default function ModelTestLeaderboardPage() {
  const params = useParams();
  const modelTestId = params.modelTestId as string;

  const { data: leaderboard = [], isLoading, error } = useQuery<LeaderboardEntry[]>({
    queryKey: ['modeltest-leaderboard', modelTestId],
    queryFn: async () => {
      const response = await axiosInstance.get(`/student-exams/leaderboard/modeltest/${modelTestId}`);
          ...
          <div className="flex items-center gap-5">
            ...
            <div className="space-y-1">
              <div className="flex items-center gap-2 mb-1">
                <Badge variant="outline" className="bg-accent/5 text-accent border-accent/20 font-black px-3 py-1 rounded-lg uppercase tracking-widest text-[10px]">
                  Model Test Arena
                </Badge>
              </div>
              ...
              <p className="text-text-secondary font-medium flex items-center gap-2">
                <FileBadge className="w-4 h-4 text-accent" /> মডেল টেস্টের শীর্ষ ১০০ মেধাবী
              </p>
            </div>
          </div>
        </div>
        ...
        {isLoading ? (
          ...
          <div className="text-center p-20 bg-bg-card/40 backdrop-blur-xl rounded-[32px] border-2 border-dashed border-border/50 space-y-6">
            ...
            <div className="space-y-2">
              ...
              <p className="text-text-secondary font-medium max-w-xs mx-auto">এখনো কেউ এই মডেল টেস্টটি সম্পন্ন করেনি।</p>
            </div>
          </div>
        ) : (
