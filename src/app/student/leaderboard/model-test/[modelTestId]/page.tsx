...
import { Trophy, Clock, Loader2, ArrowLeft, Medal, FileBadge } from 'lucide-react';
      ...
      <div className="max-w-4xl mx-auto space-y-8 pb-20">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-3xl font-display font-bold tracking-tight flex items-center gap-2">
              <Trophy className="w-8 h-8 text-yellow-500" /> Model Test Leaderboard
            </h1>
            <p className="text-text-secondary mt-1 flex items-center gap-1">
              <FileBadge className="w-4 h-4 text-accent" /> Top 100 Students in this Model Test
            </p>
          </div>
        </div>
        ...
        {isLoading ? (
          ...
          <div className="text-center p-10 text-text-secondary bg-bg-card/50 rounded-xl border border-border">
            No one has completed exams in this model test yet.
          </div>
        ) : (
              ...
              <table className="w-full text-left border-collapse">
                <thead>
                  <tr className="bg-bg-surface text-text-secondary text-xs uppercase tracking-wider">
                    ...
                    <th className="px-6 py-4 font-bold text-center">Total Score</th>
                    <th className="px-6 py-4 font-bold text-center">Total Time</th>
                  </tr>
                </thead>
