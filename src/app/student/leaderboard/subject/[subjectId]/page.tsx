...
import { Trophy, Clock, Loader2, ArrowLeft, Medal, BookOpen } from 'lucide-react';
...
export default function SubjectLeaderboardPage() {
  ...
  const { data: leaderboard = [], isLoading, error } = useQuery<LeaderboardEntry[]>({
    queryKey: ['subject-leaderboard', subjectId],
    queryFn: async () => {
      const response = await axiosInstance.get(`/student-exams/leaderboard/subject/${subjectId}`);
      ...
      <div className="max-w-4xl mx-auto space-y-8 pb-20">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-3xl font-display font-bold tracking-tight flex items-center gap-2">
              <Trophy className="w-8 h-8 text-yellow-500" /> Subject Leaderboard
            </h1>
            <p className="text-text-secondary mt-1 flex items-center gap-1">
              <BookOpen className="w-4 h-4" /> Top 100 Students in this Subject
            </p>
          </div>
        </div>
        ...
        {isLoading ? (
          ...
          <div className="text-center p-10 text-text-secondary bg-bg-card/50 rounded-xl border border-border">
            No one has completed exams in this subject yet.
          </div>
        ) : (
