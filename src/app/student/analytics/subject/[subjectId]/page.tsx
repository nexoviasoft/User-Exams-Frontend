...
import { Loader2, ArrowLeft, BookOpen, LineChart as LineChartIcon } from 'lucide-react';
...
export default function SubjectAnalyticsPage() {
  ...
  const { data: analytics = [], isLoading, error } = useQuery<AnalyticsEntry[]>({
    ...
    queryFn: async () => {
      const response = await axiosInstance.get(`/student-exams/analytics/subject/${subjectId}`);
      ...
      <div className="max-w-4xl mx-auto space-y-8 pb-20">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-2xl sm:text-3xl font-display font-bold tracking-tight flex items-center gap-2">
              <LineChartIcon className="w-6 h-6 sm:w-8 sm:h-8 text-primary" /> Subject Analytics
            </h1>
            <p className="text-text-secondary mt-1 flex items-center gap-1">
              <BookOpen className="w-4 h-4" /> Your performance history in this subject
            </p>
          </div>
        </div>
        ...
        {isLoading ? (
          ...
          <div className="text-center p-10 text-text-secondary bg-bg-card/50 rounded-xl border border-border">
            You haven't completed any exams in this subject yet.
          </div>
        ) : (
